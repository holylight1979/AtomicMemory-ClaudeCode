...
| 10 | （工具，無獨立文件） | `tools/project-tree.py`：專案根宣告檔 `.claude/project-tree.json` 的 show / explain / set-root / add-sub / standalone / claim / pick；讀取端 `lib/project_root.py`。說明見 [Install-forAI.md Step 7](../../Install-forAI.md)、[TECH.md §4.6](../../TECH.md) | 專案根, 子專案, project-tree, subs, root, standalone, claim, pick, 尋根, 宣告 |
| 11 | （工具，無獨立文件） | `tools/usage-snapshot/usage_snapshot.py`：每週二 03:30（用量刷新前）截圖 claude.ai/settings/usage；`--login` 一次登入專屬 Chrome profile、`--register` 註冊排程；截圖存公司共享 `\\192.168.100.100\暫存區\==公司人員==\holylight\CC-usage`（連不上退本機 `workflow/usage-snapshots/`）。說明見 [TECH.md 常駐可觀測表](../../TECH.md)、[DocIndex-System.md](../DocIndex-System.md) | 用量截圖, usage, token 用量, 每週, Task Scheduler, Playwright, Chrome profile, Cloudflare, claude.ai |
