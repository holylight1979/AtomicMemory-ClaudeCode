...
| **主模組 6 + shim 1**（V5 §5）| |
| `wg_core.py` | 路徑唯一真相（專案根判定委派 `lib/project_root.py`：各層 `.claude/project-tree.json` 宣告優先，無宣告退回最近四標記；`find_project_root`／`atom_io._find_project_root` 同源；SessionStart 印 `📍/❓/⚠️ [Guardian:ProjectRoot]`，resume 指紋不符重建 atom index；宣告增刪改 `tools/project-tree.py`）+ config/state IO + **token budget 單一來源**（CONTEXT_BUDGET_DEFAULT / TURN_BUDGET_LIMIT / compute_token_budget，2026-06-12 集中；兩估算器口徑見該檔註解。注意 compute_token_budget 為起始額，build_context 逐段扣減——session context −200、JIT −250——故 `[Context budget: x/y]` 的 y 常見 750/1750/2550 等扣減後值）+ **same_file_3x 覆轍白名單單一來源**（`RUT_FILE_WHITELIST_DEFAULT`/`is_rut_whitelisted`：README/_CHANGELOG/DocIndex-\*/各種 _INDEX/acceptance-\* 等高頻正常改動檔不構成覆轍證據；config `self_iteration.rut_file_whitelist` 覆寫；wg_episodic 生成端、wg_evasion 掃描端、wg_recall_miss 共用，略過必落 atom-debug log）+ log rotation + PreToolUse guards（合 wg_paths + wg_pretool_guards） |
