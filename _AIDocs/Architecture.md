...
| Stop | **ScanReport Gate**：宣告完成 + **本 session 自己 Edit/Write 的** `modified_files` 觸及 core 檔（hooks/lib/tools/rules/根層契約設定）或達 `min_files_to_block` + 缺收尾檢核標記 + 無使用者豁免 | `output_block` 硬阻擋，要求補 (a) 缺失修補清單 (b) 逃避通報 (c) token 警示 (d) 衍生暫存清單；每 session 只觸發一次（`scan_report_warned`）。共用工作樹/merged state 下他 session 改的 core 檔（`session_id` 不符）不誤觸發——entry 標作者 `session_id`、閘門只數 `own_mod_files`（未標記的 legacy entry fail-open） |
