...
| handlers/pre_tool_use.py | — | Write/Edit atom format gate + memory path block + cross-realm write block（外部專案 session 禁寫核心層子目錄+根層敏感檔）+ Bash 全域 MCP 變更閘 + SVN test block + 索引三檔合併閘 `check_merge_driver`（合併類 git 指令前自動 `--install`、續行類指令前自動 `--resolve`，warn-only）+ git commit 隱私閘 |
...
- **_atom_index.json**（JSON SoT）— 機器源真相，<!-- atom-total -->163<!-- /atom-total --> atoms 完整索引
