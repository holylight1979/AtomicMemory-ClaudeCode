...
> 讀者：在任一 repo（根層 `~/.claude` 或專案 `{proj}/.claude/memory`）遇到 git 合併、衝突、行尾問題的 CC session。人讀版在 `README.md`「多台電腦／多人同時寫記憶」。
> 程式真源：`tools/merge-atom-index.py`、`tools/normalize-eol.py`、`hooks/handlers/pre_tool_use.py::check_merge_driver`、`hooks/handlers/session_start.py::_index_conflict_advisory`、`lib/atom_io.py::write_text_lf`。本檔只講契約與 SOP，實作以碼為準。
...
| 3. resolve 備案 | 同一支 `merge-atom-index.py --resolve` | git 已停在衝突（驅動沒裝、或衝突發生在裝好之前）。**自動觸發**：PreToolUse 段含 `git rebase --continue|merge --continue|cherry-pick --continue|commit|stash pop|stash apply` 且 `git ls-files -u` 有索引三檔 → `--resolve --cwd <repo> --quiet` → `[Guardian:IndexConflict] 已自動合併並 add 索引檔：…`／`已 stage 你解好的版本：…`／`⚠ <error 或 remaining> → 手動 python ~/.claude/tools/merge-atom-index.py --resolve`。SessionStart `_index_conflict_advisory`：repo 卡在 rebase/merge/cherry-pick 且三檔未合併 → 一行提示 | `hooks/verify/verify_merge_driver_gate.py`（拆段分類、真 git 題、`auto_*:false` 不動作、子行程卡死仍 fail-open 且總耗時 ≤2.5s）；`verify_merge_atom_index.py` e2e：無驅動 rebase 卡住 → `--resolve` → `GIT_EDITOR=true git rebase --continue` 成功 |
...
- 缺鍵視為 true。兩旗標只影響 PreToolUse hook；git 自己呼叫驅動、手動 CLI 不受影響。
...
| `--resolve [--cwd <dir>] [--quiet]` | 把驅動套在索引三檔的 unmerged stage 上 | 0＝三檔已無 unmerged stage；1＝仍有殘留或錯誤 |
...
| 欄位 | 意思 |
|---|---|
| `resolved` | 語意驅動合成功、已寫回工作樹並 `git add` |
| `staged_user_version` | 工作樹已無標記且格式合法（JSON 可 parse／表頭與 key 唯一／MEMORY 目錄表在）→ 直接 `git add` 使用者版本 |
| `skipped` | 不處理並附原因：缺 `:2` 或 `:3`（一側刪檔，delete/modify）、路徑不在 `memory/` 或 `.claude/memory/`、`check-attr merge` 不是 `atomindex` |
| `remaining` | 仍是 unmerged：驅動殘留衝突（已寫回含標記結果、**不 add**）、工作樹有標記但不等於 git 原始衝突輸出（人解到一半，不碰）、無標記但格式不合法 |
| `installed` | 順手跑 `install()` 的結果 |
| `error` | 非預期錯誤字串；否則 `null` |

`--resolve` 演算法：`rev-parse --show-toplevel` 定根 → `git ls-files -u -z` 取 unmerged 路徑與實際存在的 stage → 篩三檔（basename ∈ 三檔、路徑白名單、check-attr）→ 三 stage 齊或 add/add（無 `:1` → base 空）→ 在 tmp 以 stage 重建 git 標準衝突輸出（`git merge-file -p`，標籤正規化）與工作樹 byte-compare → **相等**才以驅動結果覆蓋＋`git add`；不等的分流見上表。
...
## 支援的 shell 語法（PreToolUse 拆段器 `_git_segments`）

- 認得：`git <sub> …`、`git.exe <sub>`（大小寫不拘）、`git -C <path> <sub>`（含引號路徑 `-C "C:\My Repo"`；相對路徑以 tool cwd 解析）、前置 `cd X && git …`／`cd X; git …`（決定 run_cwd）、`;`／`&&`／`||`／`|` 切段。
...
| 專案 repo 索引三檔仍整檔衝突 | 專案記憶樹尚未釘 LF（`.gitattributes` 無 eol 規則，某機寫成 CRLF） | 專案 session 跑 `python ~/.claude/tools/normalize-eol.py --memory-dir .claude/memory --write-gitattributes`（Install-forAI Step 6 有完整 prompt） |
...
- **只處理三索引檔**的 unmerged stage；atom 本體或其他檔的衝突仍由人／CC 處理。
- 專案 repo 的記憶樹 LF 需專案 session 跑一次 `normalize-eol --memory-dir --write-gitattributes`；根層 session 不碰專案檔。
