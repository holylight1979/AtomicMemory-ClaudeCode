...
│   ├── _atom_index.json                            ← V5 JSON SoT（<!-- atom-breakdown -->95 atoms：core 28 + feedback 14 + 失敗模式 2 + local 51〔Tools9/MemDev34/OS5/Continuity2/Vision1〕<!-- /atom-breakdown -->）
...
`memory/_atom_index.json` 為唯一機器源（<!-- atom-total -->95<!-- /atom-total --> atoms）。`_ATOM_INDEX.md` 改為自動生成的人類可讀 mirror，僅 fallback parser 使用。
...
全域 ~<!-- atom-total -->95<!-- /atom-total --> atoms 規模用 Vector Service 是殺雞用牛刀。V5 引入 in-memory BM25（~80 行手刻於 `wg_atoms.py`）：
