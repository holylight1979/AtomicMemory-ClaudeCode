...
│   ├── _atom_index.json                            ← V5 JSON SoT（48 atoms：core 14 + feedback 7 + 失敗模式 2 + local 25〔World4/Tools10/MemDev8/OS3〕）
...
`memory/_atom_index.json` 為唯一機器源（48 atoms）。`_ATOM_INDEX.md` 改為自動生成的人類可讀 mirror，僅 fallback parser 使用。
...
全域 ~48 atoms 規模用 Vector Service 是殺雞用牛刀。V5 引入 in-memory BM25（~80 行手刻於 `wg_atoms.py`）：
