...
│   ├── _atom_index.json                            ← V5 JSON SoT（<!-- atom-breakdown -->124 atoms：core 54 + feedback 19 + 失敗模式 1 + local 50〔Tools7/MemDev40/OS2/Vision1〕<!-- /atom-breakdown -->）
...
全域 ~<!-- atom-total -->124<!-- /atom-total --> atoms 規模用 Vector Service 是殺雞用牛刀。V5 引入 in-memory BM25（~80 行手刻於 `wg_atoms.py`）：
