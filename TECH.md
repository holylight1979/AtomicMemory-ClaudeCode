...
- 現況計數：<!-- atom-breakdown -->150 atoms：core 66 + feedback 21 + 失敗模式 2 + local 61〔Tools8/MemDev50/OS2/Vision1〕<!-- /atom-breakdown -->（marker 自動同步，勿手改）。
...
**為什麼全域層用 BM25 不用向量**：全域索引共 <!-- atom-total -->150<!-- /atom-total --> 顆（含 local realm），向量檢索是殺雞用牛刀——每次 prompt 多一次 embedding round-trip（200–500ms）與一個常駐服務依賴，換來的語意召回在這個規模下用 trigger + BM25 就夠。BM25 純 Python stdlib、~80 行手刻、無外部依賴，向量服務掛了全域檢索照常。專案層 atom 可上百且措辭多樣，才值得付向量的成本。
