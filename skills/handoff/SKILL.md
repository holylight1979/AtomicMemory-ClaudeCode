---
name: handoff
description: 跨 Session Handoff Prompt Builder — 強制 6 區塊 self-sufficient 模板，避免下 session 裸奔
---
...
## Step 3.5：對抗式自我複審 gate（交付前強制，缺一不放行）

> 結構完整（6 區塊都在）≠ 語意正確。**作者寫完不會自動發現自己的缺口**——2026-06 dogfood 實證：CC 直到被使用者提醒「認真做兩項確認」，才在自寫的 next-phase.md 找到會引發失真/錯誤認知的缺口並補上。故此 gate 強制一輪批判複審，**不能略過、不能只在心裡跑**。

逐項自問，任一「否」→ 回去補，不交付：

- [ ] 新 session 讀完知道**為何而做**（outcome / why），不只知道做什麼？
- [ ] 每個已鎖定決策都附**一行理由**？被否決的 alternative 有寫原因？
- [ ] **未解問題 / 不確定性**獨立成區，每條是「可被回答的具體問句」？
- [ ] 只寫了 **delta**（本 session 特有、會變、接手必知）？通用規則 / SOP 用 `[[連結]]` 帶過、沒整段重貼？
- [ ] load-bearing 的數值 / 路徑 / 識別碼**逐字保留**、沒被改寫成模糊描述？
- [ ] **假設 vs 已驗證**有標記？未驗證項標「待確認、勿據此鎖死」？
- [ ] 新舊矛盾**顯式裁決**（以 X 為準、作廢 Y）、沒並列丟給下游？
- [ ] 關鍵約束 / 否定條件放在**首尾**、沒被埋在中段？
