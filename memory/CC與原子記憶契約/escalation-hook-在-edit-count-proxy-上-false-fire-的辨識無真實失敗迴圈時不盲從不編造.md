...
- Related: cognitive-patterns, feedback-rigor-standards, 原子記憶審查總結-好機制被小故障卡死非過重-拔前先實證, write-raw-對未列舉-source-靜默回-okfalse-不-raise呼叫端必檢查回傳值, activation負值不是負相關-act-r對數尺度天然跨零-注入噪音修門檻與顯示勿過濾分數, 糾正與失敗偵測把sub-agent完成通知當使用者輸入-task-notification整則進ups-引用的糾正詞誤觸deeppostmortem
