...
| windows-cc-hook-閃-console-pythonw-修-layer-1勿只補巢狀-creationflags | _AIDocs/_atoms/OS/Windows/windows-cc-hook-閃-console-pythonw-修-layer-1勿只補巢狀-creationflags.md | 閃 console, console 視窗, 黑窗, 閃窗, pythonw, CREATE_NO_WINDOW, windowsHide, MCP 閃窗, hook 閃窗, WindowsTerminal, 上GIT, git commit 閃窗, subprocess spawn | global |
...
| svn-commit-中文訊息在-cp950-主控台會亂碼-必加-encoding-utf-8 | memory/版控/SVN/svn-commit-中文訊息在-cp950-主控台會亂碼-必加-encoding-utf-8.md | svn commit, svn 上版, commit 訊息, 亂碼, 中文亂碼, encoding, cp950, Big5, svn log, revprop, pre-revprop-change, 上版訊息 | global |
| msbuild-17x-重導向-stdout-輸出-utf-8-net-framework-用-encodingdefault-讀會亂碼 | memory/dotnet/msbuild-17x-重導向-stdout-輸出-utf-8-net-framework-用-encodingdefault-讀會亂碼.md | MSBuild, 亂碼, StandardOutputEncoding, Encoding.Default, Big5, cp950, RedirectStandardOutput, 編譯 log, ProcessStartInfo, UTF-8 | global |
| 刪除候選清單的進入條件要有正向資格判定-只驗exists會讓正式檔進hud刪除鈕 | _AIDocs/_atoms/MemDev/刪除候選清單的進入條件要有正向資格判定-只驗exists會讓正式檔進hud刪除鈕.md | 殘檔帳本, aec_ledger, protected_reason, (d) 衍生暫存, HUD 刪除鈕, 正式檔誤列, vcs_tracked, 受保護路徑, 刪除候選, 破壞性候選清單 | global |
| 向量庫stale清理失效根因-layer標籤含冒號拆鍵錯位-刪0列仍回報成功 | _AIDocs/_atoms/MemDev/向量庫stale清理失效根因-layer標籤含冒號拆鍵錯位-刪0列仍回報成功.md | 向量庫, vector, reindex, stale, 孤兒, orphan, LanceDB, indexer.py, _delete_stale_keys, write-gate, dedup, similar to existing atom, 幽靈 atom, rag-engine, 重複 chunk, dedupLayersFor, layers | global |
...
| testfailgate-非-pytest-失敗紀錄只靠同前綴-40-字的成功指令清除-重跑勿加-cd-前綴 | _AIDocs/_atoms/MemDev/testfailgate-非-pytest-失敗紀錄只靠同前綴-40-字的成功指令清除-重跑勿加-cd-前綴.md | TestFailGate, 測試未綠, failing_tests, 不得宣告完成, 重複觸發, Stop 閘, node --check, heredoc | global |
| git-apply-check-過但-git-am-失敗-crlf-檔案的-patch-用-git-apply-index-加-git-am-continue-保留原作者 | memory/版控/Git/git-apply-check-過但-git-am-失敗-crlf-檔案的-patch-用-git-apply-index-加-git-am-continue-保留原作者.md | git am, git apply, patch does not apply, format-patch, 同事 patch, 審核 patch, CRLF patch, git am --continue, 保留作者, 無 push 權 | global |
| always-load-規則檔修剪判準-事前規則留一句-事後且已有程式硬控制才刪-啟發式提示不算硬控制 | _AIDocs/_atoms/MemDev/always-load-規則檔修剪判準-事前規則留一句-事後且已有程式硬控制才刪-啟發式提示不算硬控制.md | core.md 太肥, 修剪規則, always-load, CLAUDE.md 瘦身, IDENTITY.md 精簡, rules/core.md, 重複規則, hook 已強制, 事前規則, 事後諸葛 | global |
| mcp-js-改動後未重啟-lazy-require-新舊模組混載-tool-回-undefined-類錯誤不是-bug-reload-window-即復原 | _AIDocs/_atoms/MemDev/mcp-js-改動後未重啟-lazy-require-新舊模組混載-tool-回-undefined-類錯誤不是-bug-reload-window-即復原.md | MCP 重啟, Reload Window, Received undefined, spawn failed, atom_write 壞, lazy-require, 模組混載, 改 js 後, paths.js | global |
| sync-memory-indexpy-在-windows-直接跑會-cp950-unicodeencodeerror-前綴-pythonioencodingutf-8 | _AIDocs/_atoms/Tools/sync-memory-indexpy-在-windows-直接跑會-cp950-unicodeencodeerror-前綴-pythonioencodingutf-8.md | sync-memory-index, cp950, UnicodeEncodeError, PYTHONIOENCODING, catalog 重生, _local_catalog, sync_doc_counts, 索引重生 | global |
| 跨層bash閘與sessionstart逾時-fd複製非寫檔-開場提醒無聲消失 | _AIDocs/_atoms/MemDev/跨層bash閘與sessionstart逾時-fd複製非寫檔-開場提醒無聲消失.md | CrossRealmBashBlock, 2>&1, fd 複製, hook_cancelled, SessionStart 逾時, timedOut, 開場提醒沒出, ScopeLayout 沒出, 一律被擋, 冷啟動 | global |
