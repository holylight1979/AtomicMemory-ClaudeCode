...
- [臨] 自己 `git add` 了哪些檔 ≠ index 裡有哪些檔。別的 session、MCP 工具（如 `atom_write`）或 hook 都可能已先 stage 東西，`git commit` 會一併帶走。**固定做法：commit 前跑 `git diff --cached --name-only` 逐行核對，或改用 `git commit -- <明確路徑>`。** 實際踩過：add 了 2 檔、commit 場報 6 檔，多出四個是另一 session 的 atom 與記憶索引，被掛在無關的 commit message 底下；已 push 後重寫歷史只會製造更多混亂。
