...
> | 範圍 | 在哪裡 | 裝什麼／記什麼 | 版控庫 |
> |------|------|------|------|
> | **根層** | `~/.claude/`（每個人自己的電腦） | 原子記憶系統本身 + 跨專案通用的根本知識（偏好、通則、工具踩坑） | **原子記憶系統自己的版控庫**（下面 §0）——每位成員各自 clone 一次 |
...
上面裝的是**根層**（原子記憶系統本身，每人各自一份）。**專案層**是另一個範圍：`{專案}/.claude/memory/` 跟著專案自己的 GIT / SVN 走，隊友 pull 專案就接上，**不需要再安裝任何東西**。
